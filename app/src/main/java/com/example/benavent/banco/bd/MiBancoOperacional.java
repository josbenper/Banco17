package com.example.benavent.banco.bd;
...
//        boolean existe = Cuenta.contains(movimientoTransferencia.getCuentaDestino());
        /* Se comprueba que la cuenta destino existe */
        if (movimientoTransferencia.getCuentaDestino() != null){
            /* Si existe, se mira si la cuenda origen puede realizar la transferencia */
            if (movimientoTransferencia.getCuentaOrigen().getSaldoActual() - movimientoTransferencia.getImporte() >= 0) {
                ...
                /* Se le resta el importe a la cuenta origen */
                ...
                miBD.getMovimientoDAO().add(movimientoTransferencia);

                /* Y se le suma el importe a la cuenta destino */
                ...
                movimientoTransferencia.setTipo(1);
                miBD.getMovimientoDAO().add(movimientoTransferencia);

                return 0;
            } else {
                /* la cuenta no puede realizar la transferencia */
                return 2;
            }
        } else {
